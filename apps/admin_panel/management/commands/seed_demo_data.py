...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        manager = CustomUser.objects.filter(company=company, system_role='manager').first()
        ...
        suppliers_data = [
            {'name': 'Kano Agro Cooperative',     'state': 'Kano',    'contact': 'Malam Sani Abubakar',  'phone': '08031234567', 'reliability_score': 92},
            {'name': 'Kaduna Farm Collective',    'state': 'Kaduna',  'contact': 'Ibrahim Musa',          'phone': '08041234567', 'reliability_score': 85},
            {'name': 'Niger State Farmers Union', 'state': 'Niger',   'contact': 'Abdullahi Bello',       'phone': '08051234567', 'reliability_score': 78},
            {'name': 'Benue Agri Producers',      'state': 'Benue',   'contact': 'Emmanuel Terver',       'phone': '08061234567', 'reliability_score': 88},
            {'name': 'Kebbi Grain Merchants',     'state': 'Kebbi',   'contact': 'Usman Gwandu',          'phone': '08071234567', 'reliability_score': 95},
            {'name': 'Zamfara Farm Network',      'state': 'Zamfara', 'contact': 'Garba Shinkafi',        'phone': '08081234567', 'reliability_score': 71},
        ]
        suppliers = []
        for s in suppliers_data:
            obj, created = Supplier.objects.get_or_create(
                ...
                defaults={
                    ...
                    'phone': s['phone'],
                    'address': f'{s["state"]} State, Nigeria',
        ...
        farms_data = [
            # Kano Agro Cooperative
            {'supplier': 0, 'name': 'Kano North Plot A', 'area': 12.5, 'lat': 12.0022, 'lng': 8.5919,  'risk': 'low',      'state': 'Kano'},
            {'supplier': 0, 'name': 'Kano North Plot B', 'area': 8.3,  'lat': 12.0150, 'lng': 8.6100,  'risk': 'low',      'state': 'Kano'},
            {'supplier': 0, 'name': 'Kano South Farm',   'area': 15.0, 'lat': 11.9800, 'lng': 8.5700,  'risk': 'standard', 'state': 'Kano'},
            # Kaduna Farm Collective
            {'supplier': 1, 'name': 'Kaduna East Farm',  'area': 20.0, 'lat': 10.5222, 'lng': 7.4383,  'risk': 'low',      'state': 'Kaduna'},
            {'supplier': 1, 'name': 'Kaduna West Plot',  'area': 11.0, 'lat': 10.5000, 'lng': 7.4000,  'risk': 'standard', 'state': 'Kaduna'},
            # Niger State Farmers Union
            {'supplier': 2, 'name': 'Minna Farm Block',  'area': 18.5, 'lat': 9.6139,  'lng': 6.5569,  'risk': 'standard', 'state': 'Niger'},
            {'supplier': 2, 'name': 'Bida North Plot',   'area': 9.0,  'lat': 9.0800,  'lng': 6.0100,  'risk': 'low',      'state': 'Niger'},
            # Benue Agri Producers
            {'supplier': 3, 'name': 'Makurdi Farm A',    'area': 22.0, 'lat': 7.7300,  'lng': 8.5200,  'risk': 'low',      'state': 'Benue'},
            {'supplier': 3, 'name': 'Gboko Plot',        'area': 14.0, 'lat': 7.3167,  'lng': 9.0000,  'risk': 'standard', 'state': 'Benue'},
            # Kebbi Grain Merchants
            {'supplier': 4, 'name': 'Birnin Kebbi Farm', 'area': 25.0, 'lat': 12.4539, 'lng': 4.1975,  'risk': 'low',      'state': 'Kebbi'},
            {'supplier': 4, 'name': 'Argungu Plot',      'area': 16.0, 'lat': 12.7400, 'lng': 4.5200,  'risk': 'low',      'state': 'Kebbi'},
            {'supplier': 4, 'name': 'Zuru Farm',         'area': 10.5, 'lat': 11.4333, 'lng': 5.2333,  'risk': 'standard', 'state': 'Kebbi'},
            # Zamfara Farm Network
            {'supplier': 5, 'name': 'Gusau Farm Block',  'area': 13.0, 'lat': 12.1704, 'lng': 6.6649,  'risk': 'high',     'state': 'Zamfara'},
            {'supplier': 5, 'name': 'Kaura Namoda Plot', 'area': 7.5,  'lat': 12.5833, 'lng': 6.5833,  'risk': 'standard', 'state': 'Zamfara'},
        ]
        farms = []
        for f in farms_data:
            polygon = {
        ...
        for i in inventory_data:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'  Inventory: {i["lot"]} — {i["qty"]}kg {i["product"]}'))
        ...
        for po in po_data:
            obj, created = PurchaseOrder.objects.get_or_create(
                reference=po['ref'], company=company,
                defaults={
        ...
        for so in so_data:
            obj, created = SalesOrder.objects.get_or_create(
                reference=so['ref'], company=company,
                defaults={
        ...
        self.stdout.write('Products: 4 | Suppliers: 6 | Farms: 14 | Inventory: 7 | POs: 9 | SOs: 8')
