import pytest
from apps.suppliers.forms import normalize_field_gps_geometry
...
def test_strips_elevation():
    ...
    result = normalize_field_gps_geometry(_poly(ring))
...
def test_simplification_triggers_above_threshold():
    ...
    assert len(ring) > 200
    result = normalize_field_gps_geometry(_poly(ring), max_vertices=200)
    ring_out = result["coordinates"][0]
...
# ── Real field GPS sample ──────────────────────────────────────────────────────

def test_real_field_gps_polygon():
    ...
    result = normalize_field_gps_geometry({"type": "Polygon", "coordinates": [ring]})
    ring_out = result["coordinates"][0]
